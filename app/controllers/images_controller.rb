class ImagesController < ApplicationController
  ...
  def update
    ...
    if new_primary
      render json: Image.where(:klass => params[:klass], Common.klass(params[:klass]) => Common.klass_id(params,User.user_id(token_and_options(request))), :primary => true), status: :created
    else
  ...
  def destroy
    ...
    if image
      if image.destroy
        render json: "Image Successfully Deleted", status: :accepted
      else
        render json: image.errors.full_messages.to_sentence, status: :unprocessable_entity
      end
    else
      render json: "That image does not exist", status: :unprocessable_entity
    end
  end
end
